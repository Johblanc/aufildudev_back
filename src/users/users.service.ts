...
export class UsersService {
  ...
  async findOneByMail(email: string) {
    return await User.findOneBy({ email: email }) /* Comparer l'email communiqué par l'utilisateur... */
  }

  async create(createUserDto: CreateUserDto) {
    const user = await User.create({ ...createUserDto }).save(); /*... S'il n'existe pas déjà (Controller), créer un nouvel utilisateur */
    return user;
  }
...
if (user !== null) {
  if (updateUserDto.email) user.email = updateUserDto.email             /* Si la Dto (le front) change l'email, alors... */
  ...
  async promoteUser(promoteAdminDto: PromoAdminDto): Promise<User | null> {
    const user = await this.findOneByPseudo(promoteAdminDto.pseudo)          /* Trouve l'utilisateur par son pseudo */
    
    if (user !== null) {
      user.access_lvl = promoteAdminDto.access_lvl;                          /* Change le niveau d'accès de l'utilisateur */
